// ====== Countdown Timer ======
const exams = [
  { name: "Pharmacology (Written)", start: "2025-12-03T10:00:00", end: "2025-12-03T13:00:00" },
  { name: "Forensic Medicine (Central OSPE)", start: "2025-12-09T10:00:00", end: "2025-12-09T13:00:00" },
  { name: "Pharmacology (Central OSPE)", start: "2025-12-13T10:00:00", end: "2025-12-13T13:00:00" }
];
...
function showVivaForRange(range) {
  ...
  vivaForRange.forEach((viva, index) => {
    ...
    const interval = setInterval(() => {
      const now = new Date();
      let gap = dateObj - now;
      if(gap <= 0){ clearInterval(interval); timerEl.innerHTML = "Viva is ongoing or completed!"; return; }
      const days = Math.floor(gap / (1000*60*60*24));
      const hours = Math.floor((gap % (1000*60*60*24)) / (1000*60*60));
      const minutes = Math.floor((gap % (1000*60*60)) / (1000*60));
      const seconds = Math.floor((gap % (1000*60)) / 1000);
